...
public class CalServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 * jsp는 MVC패턴에서 View 역할을 할 것임
		 * 그러기 위해서는 Servlet에서 JSP로 결과값을 보내주어야 함.
		 * 앞으로 JSP로 데이터를 보내주는 코드를 작성할것임 */
		// 1. JSP에서 쓸 데이터를 request에 저장 (reqest는 전역변수처럼 사용,
		//	  , 컨테이너가 동작하는 동안 생성되어 있으므로)
		request.setAttribute("result", result);
		// 2. 어떤 JSP에서 request에 set한 데이터를 사용할지 지정해줌(JSP파일은 많이 존재할테니까)
